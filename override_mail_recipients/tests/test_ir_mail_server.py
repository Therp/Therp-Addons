...
class TestIrMailServer(SingleTransactionCase):
    ...
    def test_valid_override(self):
        self.env.ref("override_mail_recipients.override_email_to").value = "t@test.com"
        self.env["ir.mail_server"].patch_message(self.message)
        self.assertEqual(self.message["to"], '"to(at)example.com" <t@test.com>')
        self.assertEqual(self.message["cc"], '"cc(at)example.com" <t@test.com>')
    ...
    def test_disable_override(self):
        self.env.ref("override_mail_recipients.override_email_to").value = "disable"
        self.env["ir.mail_server"].patch_message(self.message)
        self.assertEqual(self.message["to"], "to@example.com")
        self.assertEqual(self.message["cc"], "cc@example.com")

    def test_domain_whitelist(self):
        self.env.ref("override_mail_recipients.override_email_to").value = "disable"
        self.env.ref("override_mail_recipients.domain_whitelist").value = "example.com"
        self.env["ir.mail_server"].patch_message(self.message)
        self.assertEqual(self.message["to"], "to@example.com")
        self.assertEqual(self.message["cc"], "cc@example.com")
        # bcc field will be fully deleted.
        self.assertEqual(self.message["bcc"], None)
    ...
    def test_do_replacement(self):
        ...
        self.assertEqual(replacement, '\\"Jansen  Pietersen\\" [jp(at)example.com]')
