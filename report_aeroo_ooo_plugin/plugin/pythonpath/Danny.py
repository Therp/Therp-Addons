###########################################################################
#
# A collection of helper functions by Danny Brewer
# as extracted from the original OpenERP report designer plugin
#
#
...
class ErrorDialog:
    def __init__(self,sErrorMsg, sErrorHelpMsg="",sTitle="Error Message"):
        self.win = DBModalDialog(50, 50, 150, 120, sTitle)
        self.win.addFixedText("lblErrMsg", 5, 5, 120, 10, sErrorMsg)
        self.win.addEdit(
            "lblErrHelpMsg", 5, 15, 140, 75, sErrorHelpMsg,
            cAutoVScroll=True, cReadOnly=True, cMultiline=True)
...
class DBModalDialog:
    ...
    def addControl( self, cCtrlServiceName,
        ...
        if cReadOnly != None:
            oControlModel.ReadOnly = True

        if cAutoVScroll != None:
            oControlModel.AutoVScroll = True
