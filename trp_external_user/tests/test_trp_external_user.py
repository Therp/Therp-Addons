...
class TestTrpExternalUser(TransactionCase):
    def test_trp_external_user(self):
        ...
        with self.assertRaises(AccessError):
            self.env.ref('base.partner_root').sudo(external_user).name
        with self.assertRaises(AccessError):
            self.env.ref('base.res_partner_1').sudo(external_user).name
        with self.assertRaises(AccessError):
            self.env.ref('base.user_root').sudo(external_user).name
        self.assertEqual(
            self.env.ref('base.main_partner').name,
            self.env.ref('base.main_partner').sudo(external_user).name,
        )
        self.assertEqual(
            self.env.ref('base.res_partner_2').name,
            self.env.ref('base.res_partner_2').sudo(external_user).name,
        )
        self.assertEqual(
            external_user.name, external_user.sudo(external_user).name,
        )
        # check flag
