...
class report_result(osv.osv):
    ...
    def read(self, cr, user, ids, readfields=None, context=None, load='_classic_read'):
        """
        ...
        # Retrieve many2one (id, name) representations
        # For graph view
        i = 0
        for field in report.field_ids:
            if field.field_id.model:
                model = self.pool.get(field.field_id.model)
                column_name = field.field_id.name
                if isinstance(model._columns[column_name], fields.many2one):
                    model2 = self.pool.get(model._columns[column_name]._obj)
                    sql_column = 'field' + str(i)
                    names = dict(
                        model2.name_get(
                        cr, user, [
                            row[sql_column] for row in result
                            ], context=context))
                    for row in result:
                        row[sql_column] = (
                            row[sql_column], names.get(
                                row[sql_column], 'Unknown')
                            )
                i += 1
        return result
